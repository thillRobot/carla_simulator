...
##### Step 2 New Option -  JHMeusener Projection Workaround + OpenStreetMap (.osm) to OpenDRIVE format (.xodr) conversion using CARLA
...
###### Convert the prepared .osm to .xodr with `convert_osm_xodr.py` from CARLA

Edited the input and output file paths in the script `convert_osm_xodr.py`. Notice they have the strange directory backouts similar to above, but there must be a better way, this is discussed in maps.md and issues.md.
...
Run the script. You might see some warnings like shown below. The file `<MAP_NAME>_CARLA.xodr` should be generated.
```
