...
Export a .osm file using with the [web app](www.openstreetmap.org). Choose a bounding box (lattitude, longitude) and record these values. You may need them later. Save or export the OpenStreetMap data as a **.osm** file. I suggest saving the infomation for each map in a file called `<map_name>_info.md`
